...
";
        ...
        // should report 2 valid passports - the first and third
...
iyr:2011 ecl:brn hgt:59in";
        ...
        public static IEnumerable<Passport> Parse(string input)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    var result = ParsePassport(currentBatch);
            ...
            // at the end, if there are any lines left, yield those as well
            if (currentBatch.Any())
            {
                yield return ParsePassport(currentBatch);
            }
        }

        public static Passport ParsePassport(List<string> lines)
        {
            ...
            var newLines = linesString.Split("\n", StringSplitOptions.None);

            // and convert it into a key-value pair with the field abbreviation
            var pieces = newLines.ToList().Select(x => new KeyValuePair<string, string>(x.Split(":").First(), x.Split(":").Last()));
            ...
            foreach(var kv in pieces)
            {
                ...
                {
                    case "byr":
                        passport.BirthYear = kv.Value;
                        break;
                    case "iyr":
                        passport.IssueYear = kv.Value;
                        break;
                    case "eyr":
                        passport.ExpirationYear = kv.Value;
                        break;
                    case "hgt":
                        passport.Height = kv.Value;
                        break;
                    case "hcl":
                        passport.HairColor = kv.Value;
                        break;
                    case "ecl":
                        passport.EyeColor = kv.Value;
                        break;
                    case "pid":
                        passport.PassportId = kv.Value;
                        break;
                    case "cid":
                        passport.CountryId = kv.Value;
                        break;
                }
            }
        ...
        public static bool IsValid(Passport p)
        {
            if (!String.IsNullOrWhiteSpace(p.BirthYear) &&
