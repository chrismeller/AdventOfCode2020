...
";
        ...
        {
            var passwords = Input.Trim().Split(new string[] { "\n" }, StringSplitOptions.None).Select(x =>
            {
                var line = x.Trim();

                var policyPart = line.Split(':').First();

                var minMaxPart = policyPart.Split(' ').First();
                var min = minMaxPart.Split('-').First();
                var max = minMaxPart.Split('-').Last();

                var letter = policyPart.Split(' ').Last();

                var password = line.Split(':').Last().Trim();

                return new PasswordPolicy()
                {
                    Min = Convert.ToInt16(min),
        ...
        private static bool IsCompliant(PasswordPolicy password)
        {
            var letters = password.Password.ToCharArray().Select(x => Convert.ToString(x));

            var count = letters.Count(x => x == password.Letter);
